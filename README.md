...
- **Release** (`release.yml`) — on every pushed tag: automatically create a GitHub
  Release for that tag with autogenerated release notes, attaching a CycloneDX SBOM
  (`bom.json`) and an `image-digests.txt` file with immutable GHCR image references.
